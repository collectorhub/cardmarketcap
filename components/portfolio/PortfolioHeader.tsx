...
export default function PortfolioHeader({ data }: { data: any }) {
  ...
  // Robust check for watchlist mode
  const isWatchlist = pathname === '/portfolio/watchlist';
  const mode = isWatchlist ? 'watchlist' : 'portfolio';

  // --- LOGIC PRESERVATION ---
  // If data comes from watchlist.php, it's under 'data.watchlist'
  // If data comes from portfolio.php, it's usually under 'data' or 'data.stats'
  // Adjust these selectors based on your specific page-level props
  const stats = isWatchlist ? data : data?.stats; 
  
  const totalValue = stats?.totalValue || 0;
  const growthPercentage = stats?.growth7D || 0;
  const cardCount = stats?.totalCards || 0;
  const setCount = stats?.setCount || data?.stats?.totalSets || 0;
  ...
  const growthAmount = (totalValue * (growthPercentage / 100)).toLocaleString(undefined, { 
    minimumFractionDigits: 2, 
    maximumFractionDigits: 2 
  });

  const headerTitle = isWatchlist ? "Your Watchlist" : `Welcome back, ${userName}! 👋`;
  const subTitle = isWatchlist 
    ? "Monitoring market moves for your tracked assets."
  ...
  return (
    <header className="w-full pt-20 md:pt-8 pb-0 px-4 md:px-0">
        ...
        <div className="w-full sm:w-auto bg-white dark:bg-slate-900 px-5 py-4 rounded-[20px] border border-slate-100 dark:border-slate-800 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-row items-center justify-between sm:justify-start gap-4 sm:gap-10">
          <div className="flex flex-col items-start gap-1">
            <div className="flex items-center gap-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-[0.05em]">
                {isWatchlist ? "Market Value" : "Total Value"}
              </p>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-50/50 dark:bg-emerald-500/10 text-emerald-500 border border-emerald-100 dark:border-emerald-500/20">
                ...
                <span className="text-[10px] font-bold">{growthPercentage}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-6 sm:gap-8">
            <div className="flex flex-col gap-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-[0.05em]">
                {isWatchlist ? "Watching" : "Cards"}
              </p>
              <p className="text-[18px] md:text-[22px] font-black text-slate-900 dark:text-white leading-none">{cardCount}</p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-[0.05em]">Sets</p>
              <p className="text-[18px] md:text-[22px] font-black text-slate-900 dark:text-white leading-none">{setCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <AddCardModal 
            mode={mode} // PASSING THE DYNAMIC MODE HERE
            userId={userId} 
