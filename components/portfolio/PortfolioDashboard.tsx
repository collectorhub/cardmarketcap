...
export default function PortfolioDashboard({ data }: { data: any }) {
  // Destructure the mapped data with fallbacks
  const { 
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[24px] border border-slate-100 dark:border-slate-800 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.02)] transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] group/card">
            <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-4">
              <div>
               ...
               {/* Replace hardcoded $48,725.60 and +12.48% */}
      ...
      <span>May 18</span>
              ...
              <div className="md:col-span-5 flex flex-col justify-between border-l border-slate-50 dark:border-slate-800 pl-0 md:pl-8 space-y-3">
                ...
                {[
                  { 
                    label: 'Change (7D)', 
                    val: `$${(performance?.change7D ?? 0).toLocaleString()}`, 
                    status: (performance?.change7D ?? 0) >= 0 ? 'pos' : 'neg' 
                  },
                  { 
                    label: 'All Time High', 
                    val: `$${(performance?.allTimeHigh ?? 0).toLocaleString()}`, 
                    status: 'neutral' 
                  },
                  { 
                    label: 'All Time Low', 
                    val: `$${(performance?.allTimeLow ?? 0).toLocaleString()}`, 
                    status: 'neutral' 
                  },
                  { 
                    label: 'Best Performing Card', 
                    val: cards[0]?.name || 'N/A', 
                    sub: cards[0]?.change > 0 ? `+${cards[0].change}%` : '0%', 
                    status: 'pos' 
                  },
                ].map((item, i) => (
