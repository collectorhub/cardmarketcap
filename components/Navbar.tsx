...
import { 
  ...
  Briefcase, ArrowRight, Home, UserPlus, LogIn 
} from 'lucide-react'
...
export default function Navbar() {
  ...
  return (
    <>
      {/* --- MOBILE TOP HEADER (New Toggler Location) --- */}
            ...
            {isLandingPage ? (
              <div className="flex items-center gap-4">
                ...
                <Button className="bg-[#00BA88] hover:bg-[#00a377] text-white font-bold rounded-xl px-6 py-3 shadow-lg shadow-emerald-500/20 active:scale-95" asChild>
                  <Link href="/overview" className="flex items-center">Get Started <ArrowRight className="ml-2 h-4 w-4" /></Link>
                </Button>
              </div>
            ) : (
              <DropdownMenu>
      ...
      {/* --- MOBILE TAB BAR --- */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-950/95 backdrop-blur-2xl border-t border-slate-200 dark:border-slate-800 h-16 pb-safe flex items-center shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <MobileTab href="/" icon={Home} label="Home" />
        <MobileTab href="/portfolio" icon={Briefcase} label="Portfolio" />
        <MobileTab onClick={() => console.log("Open Search")} icon={Search} label="Search" />
        {isLandingPage ? (
          <>
