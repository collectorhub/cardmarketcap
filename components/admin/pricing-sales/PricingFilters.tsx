...
import { Layers, AlertTriangle, ShieldCheck, ArrowUpRight, LucideIcon } from 'lucide-react';
import { cn } from "@/lib/utils";

interface PricingFiltersProps {
  activeTab: 'all_sales' | 'anomalies' | 'verified_pricing';
  setActiveTab: (tab: 'all_sales' | 'anomalies' | 'verified_pricing') => void;
  searchQuery: string;
...
export default function PricingFilters({
  ...
  const tabs: { id: 'all_sales' | 'anomalies' | 'verified_pricing'; label: string; icon: LucideIcon }[] = [
    { id: 'all_sales', label: 'All Sales Logs', icon: Layers },
    { id: 'anomalies', label: 'Anomalies & Spikes', icon: AlertTriangle },
    { id: 'verified_pricing', label: 'Verified Pricing Pool', icon: ShieldCheck },
  ];
  ...
  return (
    <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 w-full pt-1">
      ...
      <div className="relative w-full lg:w-96 font-inter">
        ...
        <input 
          type="text"
          placeholder="Filter transactions by card string name..."
          value={searchQuery}
      ...
      {/* Right Side Compact Filter Tabs Container Layout */}
